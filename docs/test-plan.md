...
```powershell
cd D:\AI\WindowsApp\shot-progress-asset-dependency-manager
npm test
npm run validate
npm run build
```
...
- `npm run validate` で `dist/shot-progress-asset-dependency-manager-representative-report.json` を生成する。
